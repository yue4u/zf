...
guides_and_hints!(
    @StartMenu where
    guide: "Type `tutorial` to continue or `help` for help.",
    hint: [
        "tutorial"
    ],

    @TutorialEngine where
    guide: "Type `engine --help` to explore the engine command",
    hint: [
        "engine thruster 100"
    ],

    @TutorialEngineRel where
    guide: r#"Type `engine rel --help` to explore the engine rel command
...
"#,
    ...
    guide:
        r#"We can use `|` to pipe data from one command to another!
...
"#,
    ...
    @TutorialFire where
    guide: r#"let's use our weapon system!
