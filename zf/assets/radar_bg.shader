// adapted from https://godotshaders.com/shader/set-corner-radius-for-texture/
...
void fragment() 
{
  ...
  else 
  {
    ...
    COLOR = vec4(0.0, 0.9, 1.0, .1);
  	if (mod(FRAGCOORD.x, grid_size) < 1. || mod(FRAGCOORD.y, grid_size) < 1.) {
